...
function isSkippableMeasureLine(line) {
...
function isMeasureBodyLine(line) {
  const trimmed = String(line || "").trim();
...
function findMeasureStartOffsetByNumber(text, measureNumber) {
  ...
  for (let li = 0; li < lineStarts.length - 1; li += 1) {
    ...
    if (isSkippableMeasureLine(rawLine)) continue;
    if (!started && !isMeasureBodyLine(rawLine)) continue;
    if (!started) {
      started = true;
...
function normalizeVoiceIdForMeasureNavigation(value) {
  return String(value || "")
    .trim()
    .split(/\s+/)[0]
    .replace(/^"|"$/g, "");
...
function findMeasureStartOffsetByNumberInPrimaryVoice(text, measureNumber) {
  ...
  for (let li = 0; li < lineStarts.length - 1; li += 1) {
    ...
    if (voiceLine) {
      currentVoice = normalizeVoiceIdForMeasureNavigation(voiceLine[1]) || "1";
      if (!primaryVoice) primaryVoice = currentVoice;
      continue;
    }

    const effectiveVoice = currentVoice || "1";
    if (!primaryVoice && isMeasureBodyLine(rawLine)) primaryVoice = effectiveVoice;
